<script lang="ts">
  ...
  const categories = [
    ...
    { value: 'sonstige', label: 'Sonstige', icon: Box, color: 'text-slate-400 bg-slate-500/10 border-slate-500/30' },
  ];
...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between border-b border-slate-800 pb-4">
    <div>
      <h2 class="text-xl font-bold text-white font-outfit">Hardware-Katalog</h2>
      <p class="text-xs text-slate-500">Verfügbare Komponenten für Rack-Einbau</p>
    </div>
    <button onclick={openAdd}
      class="flex items-center space-x-2 px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-xs font-semibold transition">
  ...
  <div class="flex flex-wrap gap-2">
    <button onclick={() => { filterCategory = 'all'; loadData(); }}
      class="px-3 py-1.5 rounded-lg text-xs font-medium transition {filterCategory === 'all' ? 'bg-slate-700 text-white' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'}">
      Alle
    </button>
    {#each categories as cat}
      <button onclick={() => { filterCategory = cat.value; loadData(); }}
        class="px-3 py-1.5 rounded-lg text-xs font-medium transition flex items-center gap-1.5 {filterCategory === cat.value ? 'bg-slate-700 text-white' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'}">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-[#101622] border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition group">
          <div class="flex items-start justify-between mb-3">
            ...
            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition">
              <button onclick={() => openEdit(item)} class="p-1.5 bg-slate-800 hover:bg-slate-700 rounded text-slate-400 transition"><Edit2 class="w-3 h-3" /></button>
          ...
          {#if item.hersteller}
            <p class="text-[11px] text-slate-500 mb-3">{item.hersteller}{item.modell ? ' ' + item.modell : ''}</p>
          {/if}

          {#if item.kategorie === 'rack'}
            <div class="grid grid-cols-3 gap-2 text-center">
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">Höhe</div>
                <div class="text-sm font-bold text-white">{item.u_hoehe} HE</div>
              </div>
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">Breite</div>
                <div class="text-sm font-bold text-white">{item.breite_mm ?? '—'} mm</div>
              </div>
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">Tiefe</div>
                <div class="text-sm font-bold text-white">{item.tiefe_mm ?? '—'} mm</div>
              </div>
            </div>
          {:else if item.kategorie === 'pdu'}
            <div class="grid grid-cols-2 gap-2 text-center">
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">Höhe</div>
                <div class="text-sm font-bold text-white">{item.u_hoehe === 0 ? 'Zero-U' : item.u_hoehe + ' HE'}</div>
              </div>
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">Mind. Rack</div>
                <div class="text-sm font-bold text-white">{item.min_rack_hoehe ?? '—'} HE</div>
              </div>
            </div>
          {:else if item.kategorie === 'usv'}
            <div class="grid grid-cols-3 gap-2 text-center">
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">Leistung</div>
                <div class="text-sm font-bold text-white">{item.leistung_kw ?? (item.tdp_watt ? item.tdp_watt / 1000 : '—')} kW</div>
              </div>
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">Module</div>
                <div class="text-sm font-bold text-white">{item.psu_count ?? '—'}× {item.psu_nennwatt ?? '—'} kW</div>
              </div>
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">Höhe</div>
                <div class="text-sm font-bold text-white">{item.u_hoehe === 0 ? 'Schrank' : item.u_hoehe + ' HE'}</div>
              </div>
            </div>
          {:else if item.kategorie === 'usv_modul'}
            <div class="grid grid-cols-3 gap-2 text-center">
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">Leistung</div>
                <div class="text-sm font-bold text-white">{item.leistung_kw ?? '—'} kW</div>
              </div>
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">Höhe</div>
                <div class="text-sm font-bold text-white">{item.u_hoehe} HE</div>
              ...
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">Hot-Swap</div>
                <div class="text-sm font-bold {item.n1_faehig ? 'text-emerald-400' : 'text-slate-500'}">{item.n1_faehig ? '✓' : '—'}</div>
              </div>
            </div>
          {:else if item.kategorie === 'switch' || item.kategorie === 'kvm'}
            <div class="grid grid-cols-2 gap-2 text-center">
              ...
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">TDP</div>
                <div class="text-sm font-bold text-white">{item.tdp_watt ?? '—'} W</div>
              </div>
            </div>
          {:else}
            <div class="grid grid-cols-3 gap-2 text-center">
              ...
              <div class="bg-slate-950/60 rounded-lg p-2">
                <div class="text-[10px] text-slate-500">PSU</div>
                <div class="text-sm font-bold text-white">{item.psu_count ?? '—'}× {item.psu_nennwatt ?? '—'} W</div>
              </div>
            </div>
          {/if}

          {#if item.bemerkung}
            <p class="text-[10px] text-slate-600 mt-3 line-clamp-2">{item.bemerkung}</p>
          {/if}

          <!-- Port breakdown -->
          <div class="flex flex-wrap gap-2 mt-3 text-[10px] text-slate-500">
            ...
            {/if}
            {#if item.port_count_rj45 > 0}<span class="px-1.5 py-0.5 rounded bg-slate-900">RJ45: {item.port_count_rj45}</span>{/if}
            {#if item.port_count_lwl > 0}<span class="px-1.5 py-0.5 rounded bg-slate-900">LWL: {item.port_count_lwl}</span>{/if}
            {#if item.port_count_sfp > 0}<span class="px-1.5 py-0.5 rounded bg-slate-900">SFP: {item.port_count_sfp}</span>{/if}
      ...
      {:else}
        <div class="col-span-full text-center py-12 text-slate-500 text-sm">
          <Cpu class="w-8 h-8 mx-auto mb-2 text-slate-700" />
...
<div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
  <div class="bg-[#101622] border border-slate-800 rounded-xl p-6 max-w-lg w-full shadow-2xl max-h-[90vh] overflow-y-auto">
    ...
    <form onsubmit={handleSave} class="space-y-4">
      ...
      {#if kategorie !== 'rack'}
      <div>
        <label class="block text-xs font-semibold text-slate-400 mb-1">Ports</label>
      ...
      <div class="flex justify-end space-x-3 pt-2">
        ...
        <button type="submit"
          class="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-semibold transition">Speichern</button>
      </div>
    </form>
  </div>
</div>
