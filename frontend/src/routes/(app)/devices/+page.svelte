...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between border-b border-slate-800 pb-4">
    <div>
      <h2 class="text-xl font-bold text-white font-outfit">Rechenzentrum Geräte</h2>
  ...
  {:else}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-1 space-y-2">
          ...
          <button onclick={() => selectDevice(device)}
            ...
            <div class="flex items-center space-x-3">
              ...
              <div class="min-w-0 flex-1">
                <h4 class="font-bold text-sm text-white truncate">{device.hostname}</h4>
                <p class="text-[10px] text-slate-500 truncate">
                  <span class="font-mono uppercase bg-slate-800 px-1 py-0.5 rounded text-[8px]">{device.typ}</span>
                  {#if noRack}
        ...
        {:else}
          <div class="text-center py-8 text-slate-500 text-sm">Keine Geräte gefunden.</div>
        {/each}
      </div>
      ...
      <div class="lg:col-span-2">
          ...
          <div class="bg-[#131615] border border-slate-800 rounded-xl p-6 space-y-6">
            <div class="flex items-start justify-between">
              ...
              <div class="flex space-x-2">
                <button onclick={openEditModal} class="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400 transition"><Edit2 class="w-4 h-4" /></button>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div class="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
                <div class="text-[10px] text-slate-500 uppercase font-bold font-mono">Phase</div>
                <div class="text-lg font-bold text-white mt-0.5">{selectedDevice.phase ?? '—'}</div>
              </div>
              <div class="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
                <div class="text-[10px] text-slate-500 uppercase font-bold font-mono">Anschluss W</div>
                <div class="text-lg font-bold text-white mt-0.5">{selectedDevice.tdp_watt ?? '—'} W</div>
              </div>
              <div class="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
                <div class="text-[10px] text-slate-500 uppercase font-bold font-mono">Höhe</div>
                <div class="text-lg font-bold text-white mt-0.5">{selectedDevice.u_hoehe} HE</div>
              </div>
            </div>
            ...
            {#if ports.length > 0}
              <div>
                <h4 class="text-xs font-bold text-slate-500 uppercase tracking-wider font-mono mb-3">Ports & Verbindungen</h4>
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {#each ports as port}
                    {@const conn = getPortConnection(port.id)}
                    <div class="bg-slate-900/40 border border-slate-800/80 rounded-lg p-3 text-xs">
                      <div class="flex items-center justify-between">
                        <span class="font-mono font-bold text-slate-300">{port.port_name}</span>
                        <span class="text-[10px] px-1.5 py-0.5 rounded bg-slate-800 text-slate-400">{port.typ}</span>
                      </div>
                      {#if conn}
                        <div class="mt-1 text-[10px] text-emerald-400 truncate">→ {conn.otherDevice?.hostname || '?'}</div>
                      {:else}
                        <div class="mt-1 text-[10px] text-slate-600">Frei</div>
                      {/if}
                    </div>
                  {/each}
                </div>
        ...
        {:else}
          <div class="flex items-center justify-center h-full text-slate-500 text-sm">Gerät auswählen um Details anzuzeigen</div>
        {/if}
      </div>
    </div>
  {/if}
