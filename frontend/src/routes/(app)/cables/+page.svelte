<script lang="ts">
  ...
  const typeBadges: Record<string, { bg: string; text: string; border: string }> = {
    ...
    'DAC':         { bg: 'bg-[var(--color-border2)]',    text: 'text-[var(--color-text2)]',    border: 'border-[var(--color-border)]' },
  ...
  function getFarbeDotClass(farbe?: string): string {
    if (!farbe) return 'bg-[var(--color-border2)]';
    ...
    if (f.includes('schwarz')) return 'bg-neutral-900 border border-[var(--color-border2)]';
...
<div class="space-y-6">
  <!-- Header -->
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[var(--color-border)] pb-4">
    <div>
      ...
      <p class="text-xs text-[var(--color-text3)] mt-1">
    ...
    <div class="flex items-center gap-2">
      ...
      <a href={exportUrl('csv')} class="flex items-center gap-2 px-3 py-2 bg-[var(--color-border)] text-[var(--color-text)] border border-[var(--color-border2)] rounded-lg text-xs font-medium hover:bg-[var(--color-border2)] transition">
  ...
  {:else}
    <!-- Filters -->
    <div class="bg-[var(--color-bg2)] border border-[var(--color-border)] rounded-xl p-4 grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
      <div class="relative">
      ...
      <div class="flex items-center gap-2">
        <label for="filter-typ" class="text-[10px] uppercase font-bold tracking-wider text-[var(--color-text3)] font-mono">Typ</label>
      ...
      <div class="flex items-center gap-2">
        <label for="filter-farbe" class="text-[10px] uppercase font-bold tracking-wider text-[var(--color-text3)] font-mono">Farbe</label>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <!-- Cable List -->
      <div class="space-y-3">
        <h3 class="text-xs font-bold text-[var(--color-text3)] uppercase tracking-wider">Kabel ({filteredCables.length})</h3>
          ...
          {#each filteredCables as cable}
            ...
            <button
              ...
              class="w-full text-left p-3 rounded-xl border transition flex items-center gap-3 {
                active
                  ? 'bg-[#1D9E75]/10 border-[#1D9E75]/50 text-[var(--color-text)]'
            ...
            >
              <div class="p-2 rounded-lg {
                active ? 'bg-blue-500/20 text-blue-400'
                  ...
                  : cable.typ === 'DAC' ? 'bg-[var(--color-border2)] text-[var(--color-text2)]'
              ...
              <div class="min-w-0 flex-1">
                ...
                <div class="flex items-center gap-2 mt-0.5">
                  ...
                  <span class="text-[10px] text-[var(--color-text3)]">{cable.laenge_m}m</span>
                </div>
              </div>
            </button>
          {/each}
        ...
        {:else}
          <div class="bg-[var(--color-bg2)] border border-[var(--color-border)] rounded-xl p-6 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[var(--color-border)]">
              <div>
                <div class="flex items-center gap-3">
                  <h3 class="text-lg font-bold text-[var(--color-text)] font-outfit font-mono">{selectedCable.kabel_nr}</h3>
                  ...
                  {#if selectedCable.farbe}
                    <div class="flex items-center gap-1.5">
                      ...
                      <span class="text-xs text-[var(--color-text2)]">{selectedCable.farbe}</span>
                    </div>
                  {/if}
                </div>
                <p class="text-xs text-[var(--color-text3)] mt-1">{selectedCable.laenge_m} Meter</p>
              </div>
              <div class="flex items-center gap-2">
                <button
                ...
                <button onclick={openEditModal} class="p-2 bg-[var(--color-border)] hover:bg-[var(--color-border2)] text-[var(--color-text)] rounded-lg transition" title="Kabel bearbeiten">
                  <Edit2 class="w-4 h-4" />
                </button>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <!-- Von -->
              <div class="bg-[var(--color-bg2)] border border-[var(--color-border)] rounded-xl p-4 space-y-3">
                <div class="text-[10px] text-[var(--color-text3)] uppercase font-bold tracking-wider font-mono">Von</div>
                <div class="space-y-1">
                  <div class="text-sm font-bold text-[var(--color-text)]">{getDeviceName(selectedCable.von_device_id)}</div>
                  {#if selectedCable.von_port}
                    <div class="flex items-center gap-1.5 text-xs text-[var(--color-text2)]">
                      <Plug class="w-3 h-3" />
              ...
              <!-- Nach -->
              <div class="bg-[var(--color-bg2)] border border-[var(--color-border)] rounded-xl p-4 space-y-3">
                <div class="text-[10px] text-[var(--color-text3)] uppercase font-bold tracking-wider font-mono">Nach</div>
                <div class="space-y-1">
                  <div class="text-sm font-bold text-[var(--color-text)]">{getDeviceName(selectedCable.nach_device_id)}</div>
            ...
            <div class="flex items-center justify-center">
              <div class="flex items-center gap-3 text-[var(--color-text3)]">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div class="bg-[var(--color-bg2)] border border-[var(--color-border)] rounded-xl p-4">
                <div class="text-[10px] text-[var(--color-text3)] uppercase font-bold tracking-wider font-mono">Verlegt am</div>
                <div class="text-sm font-bold text-[var(--color-text)] mt-0.5">{selectedCable.verlegt_am || '—'}</div>
              </div>
              <div class="bg-[var(--color-bg2)] border border-[var(--color-border)] rounded-xl p-4">
                <div class="text-[10px] text-[var(--color-text3)] uppercase font-bold tracking-wider font-mono">Verlegt von</div>
                <div class="text-sm font-bold text-[var(--color-text)] mt-0.5">{selectedCable.verlegt_von || '—'}</div>
              </div>
              <div class="bg-[var(--color-bg2)] border border-[var(--color-border)] rounded-xl p-4">
                <div class="text-[10px] text-[var(--color-text3)] uppercase font-bold tracking-wider font-mono">Bemerkung</div>
                <div class="text-sm text-[var(--color-text)] mt-0.5">{selectedCable.bemerkung || '—'}</div>
              </div>
            </div>
            ...
            {#if traceResult}
              <div class="border border-violet-800/40 rounded-xl overflow-hidden">
                <div class="px-4 py-2.5 border-b border-[var(--color-border)] flex items-center justify-between bg-violet-900/10">
                  <span class="text-[10px] uppercase font-bold tracking-wider text-violet-400">Kabelpfad — {traceResult.hops} Kabel</span>
                  <button onclick={() => traceResult = null} class="text-[var(--color-text3)] hover:text-[var(--color-text2)] text-xs">✕</button>
                </div>
                <div class="p-3 space-y-1 max-h-64 overflow-y-auto">
                  {#each traceResult.trace as hop, i}
                    ...
                    <div class="flex items-center gap-2 text-xs {isSelected ? 'bg-violet-900/20 rounded px-2 py-1 border border-violet-700/30' : 'px-2 py-0.5'}">
                      <span class="text-[var(--color-text3)] shrink-0 w-4 text-right">{i + 1}</span>
                      <span class="font-mono text-[var(--color-text)] shrink-0">{hop.von_device_hostname ?? '?'}</span>
                      {#if hop.von_port}<span class="text-[var(--color-text3)] font-mono text-[10px]">:{hop.von_port}</span>{/if}
                      <span class="text-[var(--color-text2)] shrink-0">→</span>
                      <span class="font-mono text-[var(--color-text)] shrink-0">{hop.nach_device_hostname ?? '?'}</span>
                      {#if hop.nach_port}<span class="text-[var(--color-text3)] font-mono text-[10px]">:{hop.nach_port}</span>{/if}
                      <span class="ml-auto text-[10px] text-[var(--color-text3)] shrink-0">{hop.kabel_nr ?? '—'} · {hop.typ}</span>
                    ...
                    {#if i < traceResult.trace.length - 1}
                      <div class="pl-6 text-[var(--color-text)] text-[10px]">│</div>
                    {/if}
                  {/each}
                </div>
              </div>
...
<div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
  <div class="bg-[var(--color-bg2)] border border-[var(--color-border)] rounded-xl p-6 max-w-lg w-full shadow-2xl overflow-y-auto max-h-[90vh]">
    <div class="flex items-center justify-between mb-4">
      <h3 class="text-lg font-bold text-[var(--color-text)] font-outfit">Kabel hinzufügen</h3>
      <button onclick={() => showAddCable = false} class="text-[var(--color-text2)] hover:text-[var(--color-text)] transition"><X class="w-5 h-5" /></button>
    </div>
    <form onsubmit={handleAddCable} class="space-y-4">
      <div>
        <label class="block text-xs font-semibold text-[var(--color-text2)] mb-1">Kabel-Nr *</label>
      ...
      <div class="grid grid-cols-3 gap-4">
        <div>
          ...
          <select bind:value={typ} onchange={() => suggestColorForTyp(typ)} class="w-full bg-[var(--color-bg3)] border border-[var(--color-border2)] rounded-lg px-4 py-2 text-sm text-[var(--color-text)] focus:outline-none focus:border-[#1D9E75]">
            {#each cableTypes as t}<option value={t}>{t}</option>{/each}
          </select>
        </div>
        <div>
        ...
        <div>
          ...
          <select bind:value={farbe} class="w-full bg-[var(--color-bg3)] border border-[var(--color-border2)] rounded-lg px-4 py-2 text-sm text-[var(--color-text)] focus:outline-none focus:border-[#1D9E75]">
            <option value="">—</option>
            {#each farben as f}<option value={f}>{f}</option>{/each}
          </select>
        </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-semibold text-[var(--color-text2)] mb-1">Von Gerät</label>
          <select bind:value={von_device_id} class="w-full bg-[var(--color-bg3)] border border-[var(--color-border2)] rounded-lg px-4 py-2 text-sm text-[var(--color-text)] focus:outline-none focus:border-[#1D9E75]">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-semibold text-[var(--color-text2)] mb-1">Nach Gerät</label>
          <select bind:value={nach_device_id} class="w-full bg-[var(--color-bg3)] border border-[var(--color-border2)] rounded-lg px-4 py-2 text-sm text-[var(--color-text)] focus:outline-none focus:border-[#1D9E75]">
            <option value={null}>—</option>
            {#each devices as d}<option value={d.id}>{d.hostname}</option>{/each}
          </select>
        </div>
        <div>
          <label class="block text-xs font-semibold text-[var(--color-text2)] mb-1">Nach Port</label>
      ...
      <div class="flex justify-end gap-3 pt-2">
        <button type="button" onclick={() => showAddCable = false} class="px-4 py-2 rounded-lg text-sm text-[var(--color-text2)] hover:bg-[var(--color-border)] transition">Abbrechen</button>
...
<div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
  <div class="bg-[var(--color-bg2)] border border-[var(--color-border)] rounded-xl p-6 max-w-lg w-full shadow-2xl overflow-y-auto max-h-[90vh]">
    <div class="flex items-center justify-between mb-4">
      <h3 class="text-lg font-bold text-[var(--color-text)] font-outfit">Kabel bearbeiten</h3>
      <button onclick={() => showEditCable = false} class="text-[var(--color-text2)] hover:text-[var(--color-text)] transition"><X class="w-5 h-5" /></button>
    </div>
    <form onsubmit={handleEditCable} class="space-y-4">
      ...
      <div class="flex justify-end gap-3 pt-2">
        <button type="button" onclick={() => showEditCable = false} class="px-4 py-2 rounded-lg text-sm text-[var(--color-text2)] hover:bg-[var(--color-border)] transition">Abbrechen</button>
        <button type="submit" class="px-4 py-2 bg-[#1D9E75] hover:bg-[#0F6E56] text-[var(--color-text)] rounded-lg text-sm font-semibold transition">Speichern</button>
      </div>
    </form>
  </div>
...
<div class="mt-8 pt-6 border-t border-[var(--color-border)]">
  <button
    ...
    class="flex items-center gap-2 text-xs font-semibold text-[var(--color-text3)] hover:text-[var(--color-text)] transition uppercase tracking-wider"
  >
  ...
  {#if showAdmin}
    <div class="mt-3 bg-[var(--color-bg2)] border border-[var(--color-border)] rounded-xl overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full text-xs">
          <thead>
            <tr class="border-b border-[var(--color-border)] bg-[var(--color-bg2)]">
              <th class="text-left px-4 py-2 text-[var(--color-text2)] font-semibold">Typ</th>
              <th class="text-left px-4 py-2 text-[var(--color-text2)] font-semibold">Standard-Farbe</th>
              <th class="text-left px-4 py-2 text-[var(--color-text2)] font-semibold">Kategorie</th>
              <th class="text-left px-4 py-2 text-[var(--color-text2)] font-semibold">Verwendung</th>
              ...
              <tr class="hover:bg-[var(--color-border2)]">
                ...
                <td class="px-4 py-2">
                  <div class="flex items-center gap-2">
                    ...
                    {/if}
                    <span class="text-[var(--color-text)]">{rule.standard_farbe || '—'}</span>
                  </div>
                </td>
                <td class="px-4 py-2 text-[var(--color-text2)]">{rule.kategorie}</td>
                <td class="px-4 py-2 text-[var(--color-text2)]">{rule.verwendungszweck}</td>
                <td class="px-4 py-2 text-[var(--color-text3)]">{rule.standard}</td>
              </tr>
            {/each}
          </tbody>
        </table>
