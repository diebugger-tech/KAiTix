<script lang="ts">
  ...
  let editForm = $state<{ hostname: string; ip_adresse: string; ipv6_adresse: string; hersteller: string; modell: string }>({ hostname: '', ip_adresse: '', ipv6_adresse: '', hersteller: '', modell: '' });
  ...
  function handleResize(e: MouseEvent) {
    ...
    panelWidth = Math.max(280, Math.min(800, panelWidth + diff));
    startX = e.clientX;
  }
  ...
  $effect(() => {
    if (selectedNode) {
      ...
      editForm.ipv6_adresse = selectedNode.ipv6_adresse || '';
  ...
  async function saveNode() {
    ...
    try {
      const payload = {
        ...
        ipv6_adresse: editForm.ipv6_adresse.trim() || undefined,
          ...
          <div class="bg-[var(--color-bg2)] border border-[var(--color-border)] shadow-sm rounded-xl p-5 flex-1 overflow-y-auto">
            ...
            <div class="space-y-4 mb-5">
              ...
              <div>
                ...
                <input type="text" bind:value={editForm.ipv6_adresse} class="w-full bg-[var(--color-bg3)] border border-[var(--color-border2)] rounded-lg px-3 py-2 text-sm font-mono text-blue-300 focus:border-[#1D9E75] outline-none transition" placeholder="z.B. 2001:db8::1" />
              </div>
