...
class CDPClient:
    ...
    def send(self, method, params=None):
        cmd = {"id": self.message_id, "method": method, "params": params or {}}
        self.message_id += 1
...
def main():
    ...
    if not port or not runbook_id:
        print(
            "Error: Could not find KAiTix port or 'Geplanter Shutdown EG' runbook. Is KAiTix running?"
        )
        sys.exit(1)
    ...
    if not ws_url:
        print(
            "No open page tab found. Attempting to create a new page tab via browser debug URL..."
        )
        try:
    ...
    racks_count = client.evaluate(
        "Array.from(document.querySelectorAll('div')).find(el => el.textContent.includes('Serverracks'))?.querySelector('.text-2xl')?.textContent"
    )
    devices_count = client.evaluate(
        "Array.from(document.querySelectorAll('div')).find(el => el.textContent.includes('Aktive Geräte'))?.querySelector('.text-2xl')?.textContent"
    )
    ...
    report.append(
        f"1. Dashboard: {'PASS' if t1_pass else 'FAIL'} (Racks: {racks_count}, Devices: {devices_count})"
    )
    ...
    devices_visible = (
        "HE" in detail_pane_text
        or "Auslastung" in detail_pane_text
        or "Geräte" in detail_pane_text
    )
    ...
    curves_render = client.evaluate(
        "Array.from(document.querySelectorAll('svg path')).some(p => p.getAttribute('d').includes('C'))"
    )
    ...
    report.append(
        f"3. Virtual Machines (Table: {vm_rows_count} VMs, Graph SVG and curves): {'PASS' if t3_pass else 'FAIL'}"
    )
    ...
    for _ in range(20):
        is_loading = client.evaluate(
            "document.body.innerText.includes('Lade Runbook...')"
        )
        if not is_loading:
            break
        time.sleep(0.3)
    ...
    layers_count = client.evaluate(
        "Array.from(document.querySelectorAll('div')).filter(d => d.textContent.trim().match(/^[1-9]$/) && d.classList.contains('w-6') && d.classList.contains('h-6')).length"
    )
    page_text = client.evaluate("document.body.innerText")
    has_layers = (
        "Ebene 1" in page_text or "Layer" in page_text or "Web-Tier" in page_text
    )
    ...
    has_start_btn = client.evaluate(
        "Array.from(document.querySelectorAll('button')).some(b => b.textContent.includes('Starten'))"
    )
    if has_start_btn:
        print("Starting new execution in shutdown mode...")
        client.evaluate(
            "Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Starten')).click()"
        )
        time.sleep(1.5)
    ...
    step_checked = client.evaluate(
        "Array.from(document.querySelectorAll('button')).some(b => b.classList.contains('bg-emerald-500'))"
    )
    ...
    time.sleep(1.5)
    step_unchecked = not client.evaluate(
        "Array.from(document.querySelectorAll('button')).filter(b => b.classList.contains('w-6') && b.classList.contains('h-6')).some(b => b.classList.contains('bg-emerald-500'))"
    )
    ...
    t4_pass = (
        (layers_count == 4 or has_layers)
        and exec_tab
        and step_checked
        and step_unchecked
        and startup_reversed
    )
    report.append(
        f"4. Runbooks (4 layers, checklist toggle, Startup mode reversal): {'PASS' if t4_pass else 'FAIL'}"
    )
    ...
    has_excel = client.evaluate(
        "Array.from(document.querySelectorAll('a')).some(a => a.href.includes('/export/xlsx') && a.textContent.includes('Excel'))"
    )
    has_zip = client.evaluate(
        "Array.from(document.querySelectorAll('a')).some(a => a.href.includes('/export/csv') && a.textContent.includes('ZIP'))"
    )
    has_ods = client.evaluate(
        "Array.from(document.querySelectorAll('a')).some(a => a.href.includes('/export/ods') && a.textContent.includes('ODS'))"
    )
    ...
    report.append(
        f"5. Export options visible in sidebar: {'PASS' if t5_pass else 'FAIL'}"
    )
