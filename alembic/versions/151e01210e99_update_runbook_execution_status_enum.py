...
def upgrade() -> None:
    ...
    op.alter_column(
        ...
        existing_type=mysql.ENUM(
            "aktiv", "abgeschlossen", "abgebrochen", collation="utf8mb4_unicode_ci"
        ),
        type_=sa.String(50),
    ...
    op.execute(
        "UPDATE runbook_executions SET status = 'verworfen' WHERE status = 'abgebrochen'"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE runbook_executions SET status = 'abgebrochen' WHERE status = 'verworfen'"
    )
