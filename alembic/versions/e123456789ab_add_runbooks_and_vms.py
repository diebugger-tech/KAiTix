...
down_revision: Union[str, None] = "582ca8d0a3bf"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "hypervisor_typ",
            sa.Enum("vmware", "hyper-v", "kvm", "xcpng", "sonstige"),
            nullable=False,
        ),
        ...
        sa.ForeignKeyConstraint(
            ["depends_on_vm_id"], ["virtual_machines.id"], ondelete="SET NULL"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "typ",
            sa.Enum("shutdown", "startup", "wartung", "notfall", "custom"),
            nullable=False,
        ),
        ...
        sa.ForeignKeyConstraint(
            ["generated_from_id"], ["runbooks.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["layer_id"], ["runbook_layers.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(["runbook_id"], ["runbooks.id"], ondelete="CASCADE"),
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("aktiv", "abgeschlossen", "abgebrochen"),
    ...
    op.create_table(
        ...
        sa.Column("note", sa.Text(), nullable=True),
        sa.ForeignKeyConstraint(
            ["execution_id"], ["runbook_executions.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["runbook_device_id"], ["runbook_devices.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
