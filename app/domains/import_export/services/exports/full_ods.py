...
def _ods(data: dict) -> bytes:
    ...
    add_sheet("Rack-Belegung",
              ["Rack", "Standort", "U-von", "U-bis", "Hostname", "Typ", "IP",
            ...
            for d in _rack_devices(data, r["id"]):
                for iface in _ifaces_for_device(data, d["id"]):
                    ...
                    if cable:
                        ...
                        cross = "★" if (dst_dev and dst_dev.get("rack_id") != r["id"]) else ""
                        rows.append([r["name"], d["hostname"], iface.get("port", "–"),
    ...
    add_sheet("Ports & Interfaces",
              ["Rack", "Gerät", "Port", "Typ", "MAC", "Verbunden mit",
    ...
    add_sheet("Kabelliste",
              ["Kabel-Nr", "Typ", "m", "Farbe", "Von-Gerät", "Von-Rack", "Von-Port",
    ...
    def pdu_rows():
        ...
        for pdu in [d for d in data["devices"] if d.get("typ") == "pdu"]:
            ...
            for o in _pdu_outlets(data, pdu["id"]):
                ...
                rows.append([pdu["hostname"], rn, o.get("outlet_name", "–"),
    ...
    add_sheet("PDU-Belegung",
              ["PDU", "Rack", "Steckdose", "Phase", "Typ", "Max W", "Gerät", "Port", "Schaltbar"],
              pdu_rows)
