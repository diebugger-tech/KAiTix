...
from app.domains.runbooks.schemas import (
    ...
    RunbookLayerCreate,
    RunbookLayerUpdate,
    RunbookLayerReorderRequest,
    RunbookDevice,
    RunbookDeviceCreate,
    RunbookDeviceUpdate,
    RunbookDeviceReorderRequest,
    RunbookExecution,
    RunbookExecutionCreate,
    RunbookExecutionStatusUpdate,
    RunbookExecutionStep,
    RunbookExecutionStepCheckRequest,
)
...
async def list_runbooks(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(RunbookModel).options(*_runbook_options()))
...
async def get_runbook(id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(RunbookModel).where(RunbookModel.id == id).options(*_runbook_options())
    )
...
async def update_runbook(
    id: int, runbook_in: RunbookUpdate, db: AsyncSession = Depends(get_db)
):
...
async def list_layers(id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(RunbookLayerModel)
        .where(RunbookLayerModel.runbook_id == id)
        .order_by(RunbookLayerModel.position)
    )
    return result.scalars().all()


@router.post(
    "/{id}/layers", response_model=RunbookLayer, status_code=status.HTTP_201_CREATED
)
async def create_layer(
    id: int, layer_in: RunbookLayerCreate, db: AsyncSession = Depends(get_db)
):
...
async def reorder_layers(
    id: int, req: RunbookLayerReorderRequest, db: AsyncSession = Depends(get_db)
):
...
async def update_layer(
    id: int, lid: int, layer_in: RunbookLayerUpdate, db: AsyncSession = Depends(get_db)
):
    result = await db.execute(
        select(RunbookLayerModel).where(
            RunbookLayerModel.id == lid, RunbookLayerModel.runbook_id == id
        )
    )
...
@router.post(
    "/{id}/devices", response_model=RunbookDevice, status_code=status.HTTP_201_CREATED
)
async def create_runbook_device(
    id: int, device_in: RunbookDeviceCreate, db: AsyncSession = Depends(get_db)
):
...
async def reorder_devices(
    id: int, req: RunbookDeviceReorderRequest, db: AsyncSession = Depends(get_db)
):
...
async def update_runbook_device(
    id: int,
    did: int,
    device_in: RunbookDeviceUpdate,
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(
        select(RunbookDeviceModel).where(
            RunbookDeviceModel.id == did, RunbookDeviceModel.runbook_id == id
        )
    )
...
@router.post(
    "/{id}/execute",
    response_model=RunbookExecution,
...
async def check_execution_step(
    ...
    result = await db.execute(
        select(RunbookExecutionStepModel).where(
...
@executions_router.delete(
    "/{eid}/steps/{sid}/uncheck", status_code=status.HTTP_204_NO_CONTENT
)
async def uncheck_execution_step(
    eid: int, sid: int, db: AsyncSession = Depends(get_db)
):
    result = await db.execute(
        select(RunbookExecutionStepModel).where(
            RunbookExecutionStepModel.runbook_device_id == sid,
...
async def update_execution_status(
    eid: int, req: RunbookExecutionStatusUpdate, db: AsyncSession = Depends(get_db)
):
    result = await db.execute(
        select(RunbookExecutionModel).where(RunbookExecutionModel.id == eid)
    )
    execution = result.scalar_one_or_none()
    if not execution:
        raise HTTPException(status_code=404, detail="Execution not found")
    ...
    if req.status == "verworfen" and (not req.note or not req.note.strip()):
        raise HTTPException(
            status_code=400,
            detail="Eine Notiz ist für den Status 'verworfen' zwingend erforderlich",
        )
