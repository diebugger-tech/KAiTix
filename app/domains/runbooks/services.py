...
class RunbookService:
    ...
    async def generate_startup(self, id: int) -> RunbookModel:
        result = await self.db.execute(
            ...
            .options(
                selectinload(RunbookModel.layers).selectinload(
                    RunbookLayerModel.devices
                )
            )
        )
        ...
        if not shutdown_runbook or shutdown_runbook.typ != "shutdown":
            raise HTTPException(
                status_code=400, detail="Must generate from a 'shutdown' runbook"
            )
        ...
        sorted_layers = sorted(
            shutdown_runbook.layers, key=lambda x: x.position, reverse=True
        )
    ...
    async def export_markdown(self, id: int) -> str:
        ...
        lines.append(
            f"**Typ:** {runbook.typ} | **Erstellt:** {runbook.erstellt_am.strftime('%Y-%m-%d %H:%M')} | **Von:** {runbook.erstellt_von or 'System'}"
        )
        ...
        lines.append(
            ...
            + (
                ...
                else "STARTUP-SEQUENZ (umgekehrt)"
                if runbook.typ == "startup"
                else f"{runbook.typ.upper()}-SEQUENZ"
            )
        )
        ...
        for layer in sorted_layers:
            ...
            for dev in sorted_devices:
                ident = dev.freitext or (
                    dev.vm.name
                    if dev.vm
                    else (dev.device.hostname if dev.device else "Unknown")
                )
    ...
    async def export_pdf(self, id: int) -> bytes:
        ...
        typ_str = (
            ...
            if runbook.typ == "startup"
            else runbook.typ.capitalize()
        )
        erstellt_am_str = runbook.erstellt_am.strftime("%Y-%m-%d %H:%M")
        story.append(
            Paragraph(
                f"<b>Typ:</b> {typ_str} | <b>Erstellt:</b> {erstellt_am_str} | <b>Erstellt von:</b> {runbook.erstellt_von or 'System'}",
                meta_style,
            )
        )
        ...
        for layer in sorted_layers:
            story.append(
                Paragraph(f"Ebene {layer.position}: {layer.name}", layer_style)
            )
            if layer.markdown_note:
            ...
            for dev in sorted_devices:
                ...
                if dev.note:
                    ...
                    table_data.append(
                        [Paragraph("", item_style), Paragraph(note_html, note_style)]
                    )
