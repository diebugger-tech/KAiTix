...
async def migrate_racks():
    async with AsyncSessionLocal() as session:
        ...
        result = await session.execute(
            text("SELECT id, name, standort, rackreihe FROM racks")
        )
        ...
        for rack in racks:
            ...
            if standort and "RZ1-Reihe" in standort:
                ...
                print(
                    f"Updating Rack {name}: standort='{new_standort}', rackreihe='{reihe_formatted}'"
                )
                await session.execute(
                    text(
                        "UPDATE racks SET standort = :standort, rackreihe = :rackreihe WHERE id = :id"
                    ),
                    {
                        "standort": new_standort,
                        "rackreihe": reihe_formatted,
                        "id": rack_id,
                    },
                )
