...
def audit_vde_compliance(payload: dict) -> list[ValidationResult]:
    ...
    if payload.get("module_count", 0) < 2:
        results.append(ValidationResult(ValidationLevel.ERROR, "N1_IMPOSSIBLE", "Kein N+1 möglich"))
    if payload.get("battery_strings", 0) < 2:
        results.append(ValidationResult(ValidationLevel.ERROR, "BATTERY_SPOF", "SPOF Batteriesystem"))
    if not payload.get("has_epo", False):
        results.append(ValidationResult(ValidationLevel.ERROR, "EPO_MISSING", "Kein EPO-Kreis — VDE 0108-100 Verstoß"))
    if not payload.get("has_bypass", False):
        results.append(ValidationResult(ValidationLevel.WARNING, "MBS_MISSING", "Fehlender MBS — Wartung erfordert Downtime"))
    return results
    ...
    async with AsyncSessionLocal() as session:
        ...
        result = await session.execute(
            select(UsvUnit).where(
                UsvUnit.bezeichnung == usv_bezeichnung, 
                UsvUnit.rack_id == rack_id
            )
        )
        ...
        for mod in (await session.execute(select(UsvModule).where(UsvModule.usv_unit_id == usv_unit.id))).scalars().all():
        ...
        result = await session.execute(
            select(DistributionPanel).where(DistributionPanel.usv_unit_id == usv_unit.id)
        )
        ...
        for circ in (await session.execute(select(DistributionCircuit).where(DistributionCircuit.panel_id == panel.id))).scalars().all():
...
def main(
    ...
    asyncio.run(async_main(
        rack_id, usv_bezeichnung, usv_kw, module_kw, module_count,
        battery_strings, has_bypass, has_epo, phases, absicherung, dry_run, from_eplan
    ))
